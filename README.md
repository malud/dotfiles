...
- Herdr with its pi status integration, the reviewr and codex-micro plugins, plus `herdr-client` / `herdr-private` session launchers

**Linux:**
- **Ansible-based** declarative tool management
- Herdr with its pi status integration and the reviewr plugin, plus `herdr-client` / `herdr-private` session launchers
...
├── run_onchange_after_install-herdr-plugins.sh.tmpl  # Install pi integration and Herdr plugins
