...
When asked to update something Claude-related (`~/CLAUDE.md`, `.claude/`, `references/`, `languages/`), Claude MUST first check whether the file is generated by `run_after_apply-ddd-knowledge.sh.tmpl` — if it is, the script's heredoc here MUST be edited instead of the destination file.
